...
{
    _requestCitypinyin=newCity;         //_requestCitypinyin:中间变量。
}                                       //界面城市→导入→转拼音→导入中间变量←其他函数
...
//qDebug()<<"ok1";
    ...
    if (reader.hasError()) {
        ...
        if(!strData.isEmpty()){
            parseData(strData.join(splitter));      //parseData函数，解析数据
            exportDataToFile(strData.join(splitter));//exportDataToFile函数，将数据生成为文件
        }
    }
}
